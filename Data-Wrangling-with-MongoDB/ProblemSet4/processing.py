...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for i in line:
                if i in FIELDS:
                    data1[FIELDS[i]] = str(line[i].strip())
                    
            data1['label'] = re.sub('\(spider\)', '', data1['label']).strip()
            if not re.match('^[a-zA-Z0-9]+$', data1['name']) or data1['name'] == "NULL":
                data1['name'] = data1['label']
            for i in data1:
                if data1[i] == "NULL":
                    data1[i] = None
            if data1['synonym'] != None:
                temp = re.sub('[{}\*]', '', data1['synonym'])
                temp = temp.split('|')
                data1['synonym'] = temp
            data1['classification'] = {}
            for i in ['family','class','phylum','order','kingdom','genus']:
                data1['classification'][i] = data1.pop(i)
            data.append(data1)
