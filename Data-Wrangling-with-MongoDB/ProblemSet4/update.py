    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            if line['binomialAuthority_label'] != "NULL":
                data[re.sub('\(spider\)', '', line['rdf-schema#label']).strip()] = line['binomialAuthority_label']
    return data
...
def update_db(data, db):
    ...
    for i in data:
        db.arachnid.update({'label' : i},  {'$set' : {'classification' : {'binomialAuthority' : data[i]} } }, multi = True)
        print i
    updated = db.arachnid.find_one({'label': 'Opisthoncana'})
    print updated
