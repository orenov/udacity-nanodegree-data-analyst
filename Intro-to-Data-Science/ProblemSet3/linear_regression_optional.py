...
def predictions(df):
    ...
    x = df[['EXITSn_hourly' ,'maxdewpti']]#, 'mindewpti' ,'minpressurei', 'meandewpti', 'meanpressurei', 'meanwindspdi', 'mintempi', 'meantempi', 'maxtempi']]
    x = sm.add_constant(x)
    model = sm.OLS(y,x)
    results = model.fit()
    ...
    return results.fittedvalues.values
